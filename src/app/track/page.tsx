...
export default function TrackOrderPage() {
  ...
  const orderNumber = "BALMY-" + Math.floor(10000 + Math.random() * 90000);
  const [displayOrderNumber] = useState(orderNumber);
  ...
  return (
    <div className="min-h-screen flex flex-col font-tajawal dir-rtl bg-gray-50">
      ...
      <main className="flex-1 w-full max-w-4xl mx-auto px-4 py-12 md:py-20 flex flex-col items-center">
        ...
        <div className="w-full bg-white rounded-[2rem] p-8 md:p-12 shadow-xl shadow-black/[0.02] border border-gray-50 relative overflow-hidden animate-in fade-in slide-in-from-bottom-8 duration-700 delay-300">
          ...
          {currentStep < 3 && (
            <div className="mt-16 bg-gray-50 rounded-2xl p-6 flex items-start gap-4 border border-gray-100 animate-in fade-in duration-500">
              <div className="bg-gray-200/50 p-3 rounded-full text-gray-500 shrink-0">
                <Clock size={24} />
              </div>
              <div>
                <h4 className="font-bold text-[#2B3440] mb-1">الوقت المتوقع للتوصيل</h4>
                <p className="text-sm text-gray-600 leading-relaxed">
                  طلبك هيوصلك خلال <span className="font-black text-[#8c1d3b]">30 - 45 دقيقة</span> من دلوقتي. شكراً لصبرك! 🙏
                </p>
              </div>
            </div>
          )}
        ...
        <div className="w-full bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-gray-100 mt-6 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-500">
          ...
          <div className="flex flex-col gap-3">
            <div className="flex justify-between items-center text-[14px] py-2 border-b border-gray-50">
              <span className="text-gray-800 font-bold dir-ltr">{displayOrderNumber}</span>
        ...
        <div className="w-full flex flex-col sm:flex-row gap-4 mt-6 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-700">
          ...
          <a href="/track" className="flex-1 bg-white rounded-2xl p-5 shadow-sm border border-gray-100 flex items-center gap-4 hover:border-[#8c1d3b]/30 transition-colors cursor-pointer">
            <div className="bg-gray-100 p-3 rounded-full text-gray-500 shrink-0">
              <Search size={20} />
            </div>
            <div className="text-right">
              <h4 className="font-bold text-gray-900 text-[14px]">تتبع طلب آخر</h4>
              <p className="text-[12px] text-gray-500">أدخل رقم طلب مختلف</p>
            </div>
          </a>
        </div>
